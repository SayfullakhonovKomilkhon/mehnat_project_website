    ...
    <div className="border-b border-primary-700/50 md:border-0">
      ...
      >
        ...
        <ChevronDown className={cn(
          'w-5 h-5 text-primary-300 transition-transform duration-200',
          isOpen && 'rotate-180'
        )} />
      </button>
      
      {/* Desktop title */}
      <h4 className="hidden md:flex font-heading font-semibold text-base mb-4 items-center gap-2 text-white">
        <span className="w-8 h-0.5 bg-accent-gold rounded-full" />
        {title}
      </h4>
...
export function Footer({ locale }: FooterProps) {
  ...
  return (
    <footer className="bg-primary-800 text-white mt-auto relative overflow-hidden">
      {/* Background Watermark */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -right-20 -bottom-20 w-96 h-96 opacity-[0.03]">
            ...
            {!isSubscribed ? (
              <form onSubmit={handleSubscribe} className="flex gap-2 w-full md:w-auto">
                ...
                <Button
                  ...
                  className="bg-accent-gold hover:bg-accent-amber text-primary-900 px-6"
                >
                  {isSubscribing ? (
      ...
      <div className="section-container py-10 md:py-14 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 md:gap-8 lg:gap-12">
          ...
          {/* Column 1 - About */}
          <div className="py-4 md:py-0 border-b border-primary-700/50 md:border-0">
            ...
            <ul className="space-y-2.5">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  ...
                  >
                    <span className="w-0 group-hover:w-2 h-0.5 bg-accent-gold transition-all duration-200" />
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <MobileAccordion title={t('footer.contact')}>
            <ul className="space-y-3">
              <li className="flex items-start gap-3 text-sm">
                ...
                <div>
                  <span className="text-primary-200 block">{t('footer.address')}</span>
                  <a 
                    href="https://maps.google.com/?q=Tashkent,+Mustaqillik+maydoni,+5"
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <li className="flex items-center gap-3 text-sm">
                ...
                <a href="mailto:info@mehnat.uz" className="text-primary-200 hover:text-white transition-colors">
                  info@mehnat.uz
                </a>
              </li>
              <li className="flex items-start gap-3 text-sm">
                <Clock className="w-4 h-4 text-accent-gold mt-0.5 flex-shrink-0" />
        ...
        {/* Ministry Name */}
        <div className="mt-10 pt-6 border-t border-primary-700/50">
      ...
      <div className="bg-primary-900/50">
        <div className="section-container py-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            {/* Copyright */}
            <p className="text-primary-400 text-sm text-center md:text-left order-2 md:order-1">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 order-1 md:order-2">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-primary-400 hover:text-white text-sm transition-colors duration-200 inline-flex items-center gap-1"
              >
                mehnat.uz
                <ExternalLink className="w-3 h-3" />
              </a>
